...
{
	public static class RDR_Fragment
	{
		// convert fragment model to generic
		public static bool ReadFragment(MemoryStream decompMem, bool endian)
		{
			Console.WriteLine("[INFO] Red Dead Redemption Fragment");
			if (Main.useVerboseMode || Main.useVeryVerboseMode)
			{
				Console.WriteLine($"[INFO] RSC85 Res Start: 0x{FlagInfo.RSC85_ObjectStart:X8}");
			}

			// we will convert to odr format, so we skip the unneccessary sections
			...
			if (Main.useVerboseMode || Main.useVeryVerboseMode)
			{
				Console.WriteLine($"[INFO] Models count: {modelCount}");
			}
