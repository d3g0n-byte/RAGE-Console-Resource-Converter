...
{
	public static class IV_ShadingGroup
	{
		// the shaders from rdr are converted to iv here
		// for that will be a new shaders which will be used as rdr shaders
		public static string GetParam(int shaderIndex, string paramName, string[] paramsAsString)
		{
			for (int a = 0; a < RDR_ShaderManager.ShaderParams[shaderIndex].shaderparams.Length; a++)
			{
				if (paramName == RDR_ShaderManager.ShaderParams[shaderIndex].shaderparams[a].name)
				{
					return paramsAsString[a];
				}
			}
		...
		public static string ConvertRDRShaderToIV(RDR_ShaderFX fx, EndianBinaryReader br)
		{
			...
			if (!RDR_ShaderManager.ShaderNames.shaderNames.TryGetValue(fx.m_nNameHash, out string shaderName))
			{
