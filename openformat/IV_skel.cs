...
namespace Converter.openformat
{
	internal class IV_skel
	{
		public static void Build(EndianBinaryReader br, uint m_pSkeleton, string skelFileName)
		{
			...
			sbOutFileSkel.AppendLine($"Flags HaveBoneMappings HaveBoneWorldOrient AuthoredOrientation");
			...
			for (int b = 0; b < skelData.m_wBoneCount; b++)
			{
				...
				sbOutFileSkel.AppendLine($"bone {Other.ReadStringAtOffset(bone[b].m_pName, br)}");
				for (int d = 0; d < tabsCount; d++) sbOutFileSkel.Append($"\t");
				sbOutFileSkel.AppendLine($"{{");
				tabsCount++;//
				for (int d = 0; d < tabsCount; d++) sbOutFileSkel.Append($"\t");
				sbOutFileSkel.AppendLine($"Flags Invisible");
				for (int d = 0; d < tabsCount; d++) sbOutFileSkel.Append($"\t");
